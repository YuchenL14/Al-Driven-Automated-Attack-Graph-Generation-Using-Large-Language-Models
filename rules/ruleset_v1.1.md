# Attack Graph Rule Set — version 1.1
...
technique id drawn from the catalogue below. If the technique you have in mind
is not in the catalogue, it may have been renumbered in a recent ATT&CK release;
choose the closest technique that IS listed rather than leaving the field blank.
For example, where you would reach for a general "impair defenses" idea that the
catalogue no longer lists under one parent, use a listed technique such as
Disable or Modify Tools (T1685) instead. Confirm the id appears in the catalogue
before using it.
