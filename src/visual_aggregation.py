"""Collapse a fan the page cannot hold, at draw time only.

A report can put many actions at one causal depth and be right to do so. The
STOLEN PENCIL report names seven password-dumping tools, all of them reachable
once the adversary has RDP access and none of them needing any other. The
extracted graph is correct: seven events, one shared parent, one shared result.
The drawing is not, because seven boxes side by side make a page nobody reads.

Pagination cannot fix this. Dividing a fan puts the convergence -- the one
structure on that page worth seeing -- on two pages at once, and every setting
of the width budget only trades page count against page shape.

The attack-graph literature answers a wide fan by aggregating rather than
paginating. Noel and Jajodia (VizSEC/DMSEC 2004) collapse non-overlapping
subgraphs to single vertices under rules based on "common attribute values" or
"graph connectedness", so a reader moves from an overview to detail instead of
across a wall. Homer et al. (VizSec 2008) group attacks for the same reason.
The supervisor's own reference graph does exactly this by hand: "GREASE malware
executed" and "MECHANICAL malware executed" are single rectangles carrying
several technique identifiers between them.

What this module does NOT do:

  - it does not change the extracted graph. The JSON keeps all seven events,
    their evidence, their techniques and their mitigations, and remains what
    any measurement or audit reads;
  - it does not invent. The aggregate carries the union of what its members
    carried, its label states the rule that formed it rather than a summary
    somebody wrote, and every member label is reported for the legend;
  - it does not remove anything another node depends on. Only result states
    with a single producer inside the group and no consumer at all are folded
    in; the shared state the group exists to produce is left standing.

It aggregates only what a page could not have held anyway, so a fan that fits
is drawn as the model returned it.
"""
