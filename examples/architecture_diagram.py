...
# Stage 1 -- input (implemented: reports can be supplied today)
g.node("pdf", "1. Input\ncyber attack report (PDF or text)\ne.g. WannaCry, British Library, NI HSE",
       shape="folder", fillcolor=STAGE)

# Stage 2 -- ingestion (designed, next phase -> dashed)
g.node("ingest", "2. Ingestion\ndocument parsing\nPDF to clean text",
       shape="box", style="filled,rounded,dashed", fillcolor=STAGE)

# Stage 3 -- extraction (designed, next phase -> dashed), grounded by knowledge bases
...
# Contract + Stage 4 -- implemented (solid, thicker)
...
# knowledge base sits beside the extraction stage (same rank), grounding it sideways
with g.subgraph() as s:
    s.attr(rank="same")
    s.node("kb")
    s.node("llm")

# alternative output sits beside the main output (same rank)
